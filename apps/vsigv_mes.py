...
def app():
    ...
    data_form['MONTH'] = data_form['MONTH'].astype('int64')
    ...
    data_train_undummy['MONTH'] = data_train_undummy['MONTH'].astype('int64')
    ...
    data_train_undummy = data_train_undummy.sort_values(by=['MONTH'],ascending=True)


    st.write("""
    # Proyeccion de ventas por semana
    """)
    proyeccion = data_train_undummy.sort_values(by=['MONTH'],ascending=True)
    proyeccion['Año'] = 2021
    st.dataframe(proyeccion[["MONTH","Año","VSIGV","Predict"]])
    ...
    st.markdown(get_table_download_link_csv(proyeccion[["MONTH","Año","VSIGV","Predict"]]), unsafe_allow_html=True)
